...
class AgenticRAG(AgoraResearchAssistantService):
    ...
    def call_llm(self, state: MessagesState):
        response = self.llm.invoke([SystemMessage(content=self.system_instruction)] + state["messages"])
        return {"messages": response}

    def should_continue(self, state: MessagesState):
        clean_message = state["messages"][-1] # check the last cleaned message from MessageState AIMessage
        if hasattr(clean_message, "tool_calls") and clean_message.tool_calls:
            return "call_tool"
        return END
    ...
    def compile_env(self):
        ...
        workflow.add_conditional_edges("agent", self.should_continue, {"call_tool": "tools", END: END})
